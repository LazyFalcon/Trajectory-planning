...
text(t2(2,1), t2(2,2), t2(2,3),'\color{red}3  ','HorizontalAlignment','right','FontSize',13);
%%
path1 = MoveTo(robot, t1(1,:), t1(2, :), shapes, 0.9, 11);
%%
drawPath3d(path1{2}, 'k', 2);
hold on
drawPath3d(path1{3}, 'ks', 1);
...
robot.bindGP = response{1};
 ...
 %%
     response = jacobi_IK_oriented(robot, robot.bindGP, path2{2}(300,:), [0 0 -1 0] , 0.01, 500);
     response{2}
    if response{2} == 0
        gp = response{1};
    end
...
robot.bindGP = response{1};
 ...
 %% 
 que = gps_2_3_2{1}(58:1:end,:); 
%  que = gps_1_2{1};
 vps12 = computeVelocities(robot,que, 300,  true);
 %%
 drawPath3d(simulateRobotFi(robot, que))
